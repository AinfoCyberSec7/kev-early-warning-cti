...
IN_CTI_SUM = "processed/cti_cve_summary.parquet"
...
def main():
    ...
    # Days from CTI first-seen to KEV add (positive means early warning before KEV)
    enriched["lead_days_cti_to_kev"] = (
        (enriched["kev_date_added"] - enriched["cti_first_seen"]).dt.total_seconds() / 86400.0
    )
